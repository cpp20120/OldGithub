...
using namespace api;
namespace drogon {
template <> inline Users fromRequest(const HttpRequest &req) {
...
auto UsersController::newUser(
    Users &&pNewUser, function<void(const HttpResponsePtr &)> &&callback)
    -> void {
  auto callbackPtr =
      make_shared<function<void(const HttpResponsePtr &)>>(std::move(callback));
...
auto UsersController::checkInputUser(
    const Users &user, const std::function<void(const bool)> &callback)
    -> void {
  // check if missing field
  if (user.getEmail() == nullptr || user.getPassword() == nullptr ||
      user.getUsername() == nullptr) {
    callback(false);
    return;
  }
  // check if duplicate
  auto callbackPtr = make_shared<std::function<void(const bool &)>>(callback);
  userMapper.findBy(
      ...
      [callbackPtr](const std::vector<Users> &users) {
...
auto UsersController::login(
    Users &&pNewUser, std::function<void(const HttpResponsePtr &)> &&callback)
    -> void {
  auto callbackPtr = std::make_shared<function<void(const HttpResponsePtr &)>>(
      std::move(callback));
  userMapper.findOne(
      Criteria(Users::Cols::_email, CompareOperator::EQ,
               pNewUser.getValueOfEmail()),
      [=](const Users &user) {
        if (BCrypt::validatePassword(pNewUser.getValueOfPassword(),
...
auto UsersController::currentUser(
    const HttpRequestPtr &req,
    std::function<void(const HttpResponsePtr &)> &&callback) -> void {
  auto callbackPtr = make_shared<std::function<void(const HttpResponsePtr &)>>(
      std::move(callback));
  jwtService::getCurrentUserFromRequest(
      req, [callbackPtr](optional<Users> user) {
        if (user.has_value()) {
...
auto UsersController::update(
    const HttpRequestPtr &req,
    std::function<void(const HttpResponsePtr &)> &&callback, Users &&pNewUser)
    -> void {
  auto callbackPtr = make_shared<std::function<void(const HttpResponsePtr &)>>(
      std::move(callback));
  jwtService::getCurrentUserFromRequest(req, [=](std::optional<Users> user) {
    if (user.has_value()) {
      ...
      userMapper.update(
          user.value(),
          [callbackPtr, user](const size_t size) {
            auto userWithToken = UsersController::UserWithToken(user.value());
            auto json = Json::Value();
            json["user"] = userWithToken.toJson();
            auto resp = HttpResponse::newHttpJsonResponse(json);
